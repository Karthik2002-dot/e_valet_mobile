import 'package:audioplayers/audioplayers.dart';
import 'package:vibration/vibration.dart';

/// Manages the continuous alert (vibration + sound) for retrieval request
/// notifications.
///
/// Using static state ensures we never double-start or double-cancel
/// regardless of whether the trigger comes from the notification service
/// (immediate, when the push arrives) or from the UI widget (fallback,
/// when the session sheet becomes visible).
class VibrationController {
  static bool _active = false;
  static AudioPlayer? _player;

  static const String _soundAsset = 'sounds/retrieval_alert.mp3';

  /// Start a repeating vibration pattern AND play the alert sound.
  /// Idempotent — calling while already active is a no-op.
  static void startRetrievalAlert() {
    if (_active) return;
    _active = true;
    Vibration.vibrate(pattern: [0, 700, 500, 700], repeat: 0);
    _playSound();
  }

  static Future<void> _playSound() async {
    try {
      _player ??= AudioPlayer();
      await _player!.setReleaseMode(ReleaseMode.loop);
      await _player!.play(AssetSource(_soundAsset));
    } catch (_) {
      // Audio failure must never crash the app
    }
  }

  /// Cancel any ongoing vibration and stop the alert sound.
  /// Idempotent — calling while not active is a no-op.
  static void stop() {
    if (!_active) return;
    _active = false;
    Vibration.cancel();
    _stopSound();
  }

  static Future<void> _stopSound() async {
    try {
      await _player?.stop();
      await _player?.dispose();
      _player = null;
    } catch (_) {}
  }
}
